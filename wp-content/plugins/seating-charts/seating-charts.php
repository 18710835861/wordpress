...
if ( !class_exists( 'TC_Seat_Chart' ) ) {
    class TC_Seat_Chart
    {
        ...
        function woo_is_purchasable_from_product_page( $is_purchasable, $product )
        {
            ...
            if ( is_product() ) {
                $_tc_used_for_seatings = get_post_meta( $product->id, '_tc_used_for_seatings', true );
        ...
        public static function save_admin( $post_id )
        {
            ...
            if ( get_post_type( $post_id ) == 'tc_seat_charts' && isset( $_POST['tc_chart_content'] ) ) {
                ...
                $upload_dir = $upload['basedir'];
                $upload_dir = $upload_dir . '/tc-seating-charts';
                preg_match_all('/<span class="tc_seat_unit ui-selectee" id="(.*?)">/', stripslashes(minify_output($_POST['tc_chart_content'])), $tickets);
                $info = [];
                foreach ($tickets[1] as $key => $val) {
                    $info[$val] = [
                        'type' => 1
                    ];
                }
                ...
                $insert_result = $wpdb->query("insert  into w_tickets (post_id,admin_id,info) values($post_id,$admin_id,'$info')");
                if (!$insert_result) {
                    $wpdb->query("update w_tickets set info =  '$info' where post_id = $post_id");
                }
                //admin file
                $filename = $post_id . '.tcsm';
                $path = $upload_dir . '/' . $filename;
